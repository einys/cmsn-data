...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let mut valid_mouse_sessions = 0;
    ...
    writeln!(
        csv_file,
        "replay_id,session_id,move_count,entropy_score,raw_movements"
    )?;
    ...
    for line in reader.lines() {
        ...
        if let Some(json_content) = decompress_umami_replay(pure_hex) {
            if let Ok(events) = serde_json::from_str::<Vec<ReplayEvent>>(&json_content) {
                ...
                if mouse_moves.len() >= 5 {
                    valid_mouse_sessions += 1;
                    let entropy = calculate_mouse_entropy(&mouse_moves);
                    entropy_values.push(entropy);

                    // 마우스 움직임 데이터를 JSON 문자열로 변환하고 CSV 이스케이프 처리
                    let raw_moves_json = serde_json::to_string(&mouse_moves)?;
                    let escaped_json = raw_moves_json.replace("\"", "\"\"");

                    // 디코딩 및 계산된 데이터를 CSV 파일에 한 줄씩 기록
                    writeln!(
                        csv_file,
                        "{},{},{},{:.6},\"{}\"",
                        replay_id,
    ...
    println!("\n=== ✨ 분석 완료 ===");
    println!(
        "총 파싱 성공한 Gzip 리플레이 행(Row) 수: {}",
        total_processed
    );
    println!(
        "📈 최종 수집된 유효 마우스 데이터셋 세션 수: {}",
        valid_mouse_sessions
    );
    println!("📁 분석 데이터가 'mouse_entropy_analysis.csv' 파일에 저장되었습니다.");

    // 오류 처리
    if !entropy_values.is_empty() {
        draw_entropy_histogram(&entropy_values)?; // 누락되어 있던 ? 연산자 추가
    }
